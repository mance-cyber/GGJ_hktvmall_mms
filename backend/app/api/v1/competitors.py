...
async def cleanup_mock_data(
    ...
    if not confirm:
        ...
        result = await db.execute(query)
        mock_competitors = result.scalars().all()

        # 統計相關產品
        product_count = 0
        for comp in mock_competitors:
            prod_query = select(func.count(CompetitorProduct.id)).where(
        ...
        return {
            "preview": True,
            "competitors_to_delete": len(mock_competitors),
            "products_to_delete": product_count,
            "competitor_names": [c.name for c in mock_competitors],
            "message": "加上 ?confirm=true 確認刪除"
        }

    # 執行刪除
    # 1. 搵出要刪除嘅 competitor IDs
    ...
    result = await db.execute(query)
    competitor_ids = [row[0] for row in result.all()]

    if not competitor_ids:
        return {"message": "冇搵到 Mock 資料", "deleted": 0}

    # 2. 搵出相關 product IDs
    prod_query = select(CompetitorProduct.id).where(
        CompetitorProduct.competitor_id.in_(competitor_ids)
    )
    prod_result = await db.execute(prod_query)
    product_ids = [row[0] for row in prod_result.all()]

    # 3. 刪除 price_snapshots
    if product_ids:
        await db.execute(
            delete(PriceSnapshot).where(
                PriceSnapshot.competitor_product_id.in_(product_ids)
            )
        )
    ...
    if product_ids:
        await db.execute(
            delete(PriceAlert).where(
                PriceAlert.competitor_product_id.in_(product_ids)
            )
        )

    # 5. 刪除 competitor_products
    await db.execute(
    ...
    return {
        "message": "Mock 資料已清除",
        "deleted_competitors": len(competitor_ids),
