...
def get_follow_up_suggestions(intent_type: str, context: dict = None) -> List[dict]:
    ...
    # 獲取對應建議，如果沒有則返回默認
    suggestions = FOLLOW_UP_SUGGESTIONS.get(intent_key, FOLLOW_UP_SUGGESTIONS["default"])
    ...
    if context:
        ...
        if products:
            ...
            suggestions.append({"text": f"分析 {product_name} 趨勢", "icon": "📊"})

    return suggestions[:4]  # 最多返回 4 個建議
